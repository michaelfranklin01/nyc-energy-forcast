...
def shap_summary(model, X, output_dir='plots'):
    os.makedirs(output_dir, exist_ok=True)
    explainer = shap.Explainer(model.named_steps['regressor'].regressor_)
    # apply preprocessing if needed:
    X_proc = model.named_steps['regressor'].transformer_.transform(X) \
             if hasattr(model.named_steps['regressor'], 'transformer_') else X
    shap_vals = explainer(X_proc)
    shap.summary_plot(shap_vals, X_proc, show=False)
    plt.tight_layout()
...
def pdp_plot(model, X, features, output_dir='plots'):
    ...
    # If we add a trained pipeline, we can use the transformer to preprocess X
    # X_proc = model.named_steps['regressor'].transformer_.transform(X) \
    #         if hasattr(model.named_steps['regressor'], 'transformer_') else X
    display = PartialDependenceDisplay.from_estimator(
        model, X, features, ax=ax, kind='average'
    )
    plt.tight_layout()
